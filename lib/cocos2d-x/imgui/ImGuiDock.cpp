...
namespace ImGuiDock
{
	...
	void Dockspace::_renderContainer(unsigned& idgen, Container* container, ImVec2 size, ImVec2 cursorPos)
	{
		...
		else
		{
			...
			if (split0 && split1)
			{
				...
				calculated_size0 = ImVec2(container->verticalSplit ? acontsizeX : size.x, !container->verticalSplit ? acontsizeY : size.y);
				calculated_size1 = ImVec2(container->verticalSplit ? bcontsizeX : size.x, !container->verticalSplit ? bcontsizeY : size.y);
			}
			if (split0)
			{
				if (split0 == nullptr)
					size.x = 1;
				_renderContainer(idgen, split0, calculated_size0, calculatedCursorPos);
				if (container->verticalSplit)
				{
					calculatedCursorPos.x = calculated_size0.x + calculatedCursorPos.x + splitterButtonWidth;
				}
				else
				{
					calculatedCursorPos.y = calculated_size0.y + calculatedCursorPos.y + splitterButtonWidth;
				}
			}
			if (split1)
			{
				...
				if (ImGui::IsItemActive())
				{
					...
					if (mouse_delta != 0.0f)
					{
						if (split0 && split0->alwaysAutoResize != true)
						{
							...
							if (split0->size == 0)
								split0->size = container->verticalSplit ? calculated_size1.x : calculated_size1.y;
						...
						else
						{
							...
							if (split1->size == 0)
								split1->size = container->verticalSplit ? calculated_size1.x : calculated_size1.y;
				...
				_renderContainer(idgen, split1, calculated_size1, calculatedCursorPos);
			}
		}
	}
	...
	void Dockspace::updateAndDraw(ImVec2 dockspaceSize)
	{
		...
		_renderContainer(idgen, &m_container, dockspaceSize, backup_pos);
	...
	void Dockspace::_renderTabBar(Container *container, ImVec2 size, ImVec2 cursorPos)
	{
		...
		for (auto dock : container->docks)
		{
			...
			if (dock->container->parent == &m_container && m_container.splits[1] == nullptr && dock->container->docks.size() == 1)
			{
				if (delta < -2 || delta > 2)
				{
